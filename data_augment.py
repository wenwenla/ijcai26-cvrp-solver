...
def augment(input, N_aug=8):
    if N_aug == 1:
        ...
        return augment_xy_data_by_8_fold(input)
    else:
        # assert N_aug % 8 == 0
        input_1 = augment_xy_data_by_8_fold(input)
        input_2 = augment_xy_data_by_N_fold(input, N_aug - 8)
        input = torch.cat([input_1, input_2], dim=0)
        # input = input.repeat((N_aug // 8, 1, 1))
        return input
    # elif N_aug == 8:
